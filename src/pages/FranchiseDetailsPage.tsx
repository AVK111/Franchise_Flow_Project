import React, { useState, useEffect } from 'react';
import { useParams, Link } from 'react-router-dom';
import '../styles/FranchiseDetailsPage.css';

// Mock franchise data
const franchiseData = {
  'subway': {
    id: 'subway',
    name: 'Subway',
    logo: 'data:image/png;base64,', // Your existing Subway logo
    category: 'Food & Beverage',
    location: 'Global Locations',
    investmentMin: 140000,
    investmentMax: 342000,
    description: 'Subway is one of the world\'s largest restaurant chains, known for its made-to-order sandwiches, salads, and wraps.',
    yearFounded: 1965,
    totalLocations: 40000,
    trainingSupport: [
      'Initial 2-week comprehensive training program',
      'Ongoing operational support',
      'Regular menu and product updates',
      'Marketing and promotional guidance'
    ],
    requirements: {
      netWorth: 250000,
      liquidCapital: 100000,
      experienceNeeded: 'Business management experience preferred'
    },
    benefits: [
      'Proven business model',
      'Strong brand recognition',
      'Flexible location options',
      'Comprehensive training and support'
    ]
  },
  '7-eleven': {
    id: '7-eleven',
    name: '7-Eleven',
    logo: 'data:image/png;base64,', // Your existing 7-Eleven logo
    category: 'Convenience Store',
    location: 'Multiple Locations',
    investmentMin: 450000,
    investmentMax: 1200000,
    description: '7-Eleven is the world\'s largest convenience store chain, offering a wide variety of products and services to customers 24/7.',
    yearFounded: 1927,
    totalLocations: 71100,
    trainingSupport: [
      'Comprehensive 6-week training program',
      '24/7 operational support',
      'Proprietary retail information system',
      'Regular business consulting'
    ],
    requirements: {
      netWorth: 1000000,
      liquidCapital: 250000,
      experienceNeeded: 'Retail or business management experience required'
    },
    benefits: [
      'Recognized global brand',
      '24/7 business model',
      'Proprietary product lines',
      'Advanced technology systems'
    ]
  },
  'anytime-fitness': {
    id: 'anytime-fitness',
    name: 'Anytime Fitness',
    logo: 'data:image/jpeg;base64,', // Your existing Anytime Fitness logo
    category: 'Fitness & Health',
    location: 'International',
    investmentMin: 98000,
    investmentMax: 523000,
    description: 'Anytime Fitness is a 24-hour health and fitness club franchise, providing convenient and affordable fitness options for members.',
    yearFounded: 2002,
    totalLocations: 4500,
    trainingSupport: [
      'Initial training at corporate headquarters',
      'On-site support during opening',
      'Ongoing operational guidance',
      'Marketing and sales training'
    ],
    requirements: {
      netWorth: 300000,
      liquidCapital: 100000,
      experienceNeeded: 'Business or fitness industry experience preferred'
    },
    benefits: [
      '24/7 operation model',
      'Low staffing requirements',
      'Recurring revenue model',
      'Strong support system'
    ]
  }
};

const FranchiseDetailsPage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const [franchise, setFranchise] = useState<any>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // Simulate API call
    setLoading(true);
    setTimeout(() => {
      if (id && franchiseData[id as keyof typeof franchiseData]) {
        setFranchise(franchiseData[id as keyof typeof franchiseData]);
      }
      setLoading(false);
    }, 500);
  }, [id]);

  if (loading) {
    return (
      <div className="loading-container">
        <div className="loader"></div>
        <p>Loading franchise details...</p>
      </div>
    );
  }

  if (!franchise) {
    return (
      <div className="error-container">
        <h2>Franchise Not Found</h2>
        <p>The franchise you're looking for doesn't exist.</p>
        <Link to="/franchises" className="btn btn-primary">Back to Listings</Link>
      </div>
    );
  }

  const formatCurrency = (amount: number) => {
    return new Intl.NumberFormat('en-US', {
      style: 'currency',
      currency: 'USD',
      minimumFractionDigits: 0,
      maximumFractionDigits: 0
    }).format(amount);
  };

  return (
    <div className="franchise-details-page">
      <div className="container">
        <nav className="breadcrumb">
          <Link to="/franchises">Franchises</Link> / {franchise.name}
        </nav>

        <div className="franchise-header">
          <div className="franchise-logo">
            <img src={franchise.logo} alt={`${franchise.name} logo`} />
          </div>
          <div className="franchise-title">
            <h1>{franchise.name}</h1>
            <div className="franchise-meta">
              <span className="category">{franchise.category}</span>
              <span className="location">{franchise.location}</span>
            </div>
          </div>
        </div>

        <div className="franchise-content">
          <div className="main-info">
            <section className="overview">
              <h2>Overview</h2>
              <p>{franchise.description}</p>
              <div className="quick-facts">
                <div className="fact">
                  <label>Year Founded</label>
                  <span>{franchise.yearFounded}</span>
                </div>
                <div className="fact">
                  <label>Total Locations</label>
                  <span>{franchise.totalLocations.toLocaleString()}</span>
                </div>
              </div>
            </section>

            <section className="investment">
              <h2>Investment Information</h2>
              <div className="investment-range">
                <h3>Total Investment Range</h3>
                <p>{formatCurrency(franchise.investmentMin)} - {formatCurrency(franchise.investmentMax)}</p>
              </div>
              <div className="requirements">
                <h3>Requirements</h3>
                <ul>
                  <li>Net Worth: {formatCurrency(franchise.requirements.netWorth)}</li>
                  <li>Liquid Capital: {formatCurrency(franchise.requirements.liquidCapital)}</li>
                  <li>Experience: {franchise.requirements.experienceNeeded}</li>
                </ul>
              </div>
            </section>

            <section className="support">
              <h2>Training & Support</h2>
              <ul className="support-list">
                {franchise.trainingSupport.map((item: string, index: number) => (
                  <li key={index}>{item}</li>
                ))}
              </ul>
            </section>

            <section className="benefits">
              <h2>Key Benefits</h2>
              <ul className="benefits-list">
                {franchise.benefits.map((benefit: string, index: number) => (
                  <li key={index}>{benefit}</li>
                ))}
              </ul>
            </section>
          </div>

          <aside className="action-sidebar">
            <div className="action-card">
              <h3>Interested in this franchise?</h3>
              <p>Take the next step in your entrepreneurial journey.</p>
              <Link to={`/apply/${franchise.id}`} className="btn btn-primary btn-large">
                Apply Now
              </Link>
              <button className="btn btn-outline btn-large">
                Download Information Pack
              </button>
            </div>
          </aside>
        </div>
      </div>
    </div>
  );
};

export default FranchiseDetailsPage; 